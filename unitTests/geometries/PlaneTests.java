...
//        assertEquals(new Vector(sqrt3, sqrt3, sqrt3), p.getNormal(new Point(0, 0, 1)), "wrong normal to plane");
    ...
    @Test
    void testFindIntsersections1() {
        // ============ Equivalence Partitions Tests ==============
        Plane plane = new Plane(new Point(3,0,0), new Point(0,3,0), new Point(0,0,3));
        // TC01: a simple case of one intersection
        List<Point> res1 = plane.findIntsersections(new Ray(new Point(1,2,3),new Vector(0,-1,-2)));
        ...
        assertNull(plane.findIntsersections(new Ray(new Point(4,4,4),new Vector(-1,0,1))),
                "ERROR: Ray is parallel to the plane");
        // TC12: Ray is included in the plane
        assertNull(plane.findIntsersections(new Ray(new Point(1,1,1),new Vector(-1,0,1))),
                "ERROR: Ray is included in the plane - no intersection points");
        // **** Group: Ray is orthogonal to the plane
        //TC13: Ray start before the plane
        List<Point> res2 = plane.findIntsersections(new Ray(new Point(0.5,0.5,0.5),new Vector(1,1,1)));
        ...
        assertNull(plane.findIntsersections(new Ray(new Point(3,0,0),new Vector(1,0,0))),
